...
# ============================== 调优参数 ==============================
# 窗口1的BIAS恒为0，因此从2日开始。
BIAS_WINDOWS = tuple(range(2, 61))
BIAS_UPPERS = tuple(value / 100.0 for value in range(0, 31))
# ======================================================================
...
def validate_grid(
    ...
    if not BIAS_WINDOWS or any(
        not isinstance(window, int) or window < 2 for window in BIAS_WINDOWS
    ):
        raise ValueError("BIAS_WINDOWS必须由不小于2的整数构成")
    if tuple(sorted(set(BIAS_WINDOWS))) != BIAS_WINDOWS:
        raise ValueError("BIAS_WINDOWS必须严格递增且不能重复")
    if not BIAS_UPPERS or any(
        not math.isfinite(upper) for upper in BIAS_UPPERS
    ):
        raise ValueError("BIAS_UPPERS必须由有限数值构成")
    if tuple(sorted(set(BIAS_UPPERS))) != BIAS_UPPERS:
        raise ValueError("BIAS_UPPERS必须严格递增且不能重复")
...
def precompute_filter_metrics(
    ...
    required_prices = max(
        max_window,
        backtest.VOL_RETURN_DAYS + 1
        if backtest.VOL_FILTER_ENABLED
        else 0,
    )
    ...
    for index_position, index_code in enumerate(sorted(events_by_code), start=1):
        ...
        for signal_date in sorted(events_by_code[index_code]):
            ...
            if backtest.VOL_FILTER_ENABLED:
                vol_prices = prices[
                    end - backtest.VOL_RETURN_DAYS - 1 : end
                ]
                daily_returns = [
                    later / earlier - 1.0
                    for earlier, later in zip(vol_prices, vol_prices[1:])
                ]
            ...
            latest_close = prices[end - 1]
            biases = []
            for window in windows:
                moving_average = (
                    prefix_sums[end] - prefix_sums[end - window]
                ) / window
                bias = latest_close / moving_average - 1.0
...
def precompute_volatility_membership(
    backtest: ModuleType,
    factor_universes: Mapping[str, Mapping[date, Sequence[object]]],
    metrics: Mapping[tuple[date, str], FilterMetrics],
) -> dict[str, dict[date, frozenset[str]]]:
    memberships: dict[str, dict[date, frozenset[str]]] = {
        method: {} for method in SCORE_METHODS
    }
    for method in SCORE_METHODS:
        for signal_date, members in factor_universes[method].items():
            if not backtest.VOL_FILTER_ENABLED:
                memberships[method][signal_date] = frozenset(
                    member.index_code for member in members
                )
                continue
            keep_count = math.ceil(len(members) * backtest.VOL_KEEP_TOP_RATIO)
            ordered_codes = sorted(
                (member.index_code for member in members),
                key=lambda code: (
                    -metrics[(signal_date, code)].volatility
                    if backtest.VOL_FILTER_MODE == "high"
                    else metrics[(signal_date, code)].volatility,
                    code,
                ),
            )
            memberships[method][signal_date] = frozenset(
                ordered_codes[:keep_count]
            )
    return memberships
...
def select_members(
    ...
    kept_volatility_codes: frozenset[str],
    metrics: Mapping[tuple[date, str], FilterMetrics],
    window_position: int,
    upper: float,
    ...
    eligible = [
        ...
        and (
            metrics[(signal_date, member.index_code)].biases[window_position]
            < upper
            or math.isclose(
                metrics[(signal_date, member.index_code)].biases[window_position],
                upper,
...
def build_parameter_targets(
    ...
    volatility_membership: Mapping[date, frozenset[str]],
    metrics: Mapping[tuple[date, str], FilterMetrics],
    window_position: int,
    upper: float,
    ...
    for signal_date, members in factor_universe.items():
        ...
        selected, eligible_count = select_members(
            backtest,
            members,
            volatility_membership[signal_date],
            metrics,
            window_position,
            upper,
        )
...
def run_grid(
    ...
    del price_history
    volatility_membership = precompute_volatility_membership(
        backtest,
        factor_universes,
        filter_metrics,
    )
    ...
    total_pairs = len(windows) * len(uppers)
    completed_pairs = 0

    for window_position, window in enumerate(windows):
        for upper_position, upper in enumerate(uppers):
            for method in SCORE_METHODS:
                ...
                for mode in TRADE_MODES:
                    matrices[(method, mode)][upper_position, window_position] = (
                        run_total_return(
                            backtest,
                            mode,
                            context.trading_dates,
    ...
    metadata = {
        ...
        "volatility_membership": volatility_membership,
        "prices": prices,
    }
...
def save_heatmaps(
    ...
    upper_ticks = [
        ...
        if position == 0
        or math.isclose(upper * 100 % 5, 0.0, abs_tol=1e-10)
        or position == len(uppers) - 1
    ]
    saved: list[Path] = []
    ...
    for method in SCORE_METHODS:
        for mode in TRADE_MODES:
            ...
            axis.set_xlabel("BIAS_WINDOW（交易日）", fontsize=13)
            axis.set_ylabel("BIAS_UPPER", fontsize=13)
            axis.set_xticks(window_ticks)
            axis.set_xticklabels([str(windows[index]) for index in window_ticks])
            axis.set_yticks(upper_ticks)
            axis.set_yticklabels(
                [f"{uppers[index]:.0%}" for index in upper_ticks]
            )
            axis.set_xticks(
            ...
            if (
                ...
                and current_parameter[1] in uppers
            ):
                current_x = windows.index(current_parameter[0])
                current_y = uppers.index(current_parameter[1])
                axis.add_patch(
                    Rectangle(
            ...
            axis.text(
                ...
                f"蓝框=当前BIAS({current_parameter[0]}日, "
...
def main(
    ...
    matrices, metadata = run_grid(backtest, BIAS_WINDOWS, BIAS_UPPERS)
    del metadata
