...
仅改变波动率收益窗口和波动率保留比例，BIAS及其他参数由下方参数区固定。
程序复用正式交易回测脚本的交易函数，在内存中运行完整参数网格，只输出
两个趋势公式、两种成交方式对应的四张累计收益率热力图，不生成中间文件。
"""
    ...
    for method in SCORE_METHODS:
        for mode in TRADE_MODES:
            ...
            axis.text(
                ...
                f"BIAS={backtest.BIAS_WINDOW}日/"
                f"{backtest.BIAS_MODE}/"
                f"上限{backtest.BIAS_UPPER:.0%}；"
